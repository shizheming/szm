...
// 版本：2.2
// 时间：2016.2
// 更新内容：2.1的版本我是想直接从所有的input着手验证，然后个别像密码，勾选框提供接口，写一些不一样的东西，后来发现这个太不靠谱了，因为里面还有一个input和另一个input的联系，比如说确认密码和密码联系在一起，获取验证码和手机联系在一起，或是验证顺序的问题，后一个input必须在前一个input验证正确的情况下才能允许验证，第一耦合度太高了，第二验证的条件千变万化，不单单只是自己的失焦获焦验证，没有办法在一个整体中把这些所有的不同性包括进去，简单点就是不可能每种情况都去协议中接口，不现实，所以我决定在2.2版本中降低他的权重，增加他的灵活性，而不是从整个表单入手了，是从一个单一的input对象入手去构建整个表单，从局部到整体。
// ------------------------------------------------------------
...
/*formValidation({
    ...
    form1.onsubmit = function(){
        if(!phone.check()) return false;
        if(!user.check()) return false;
    };
});*/
