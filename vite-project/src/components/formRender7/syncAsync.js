      ...
      if (key[0] === '_') {
        ...
        // 是函数
          ...
          if (isFunction(result)) {
            // babel造的孽
            if (/return step/.test(result.toString()) || /return new s.default/.test(result.toString())) {
              // async函数
              result.call(this).then(data => {
              ...
              if (isString(triggerValue) || isArray(triggerValue) || isPlainObject(triggerValue)) {
                ...
                arr.forEach((item, k) => {
                  if (isString(item)) {
                    ...
                    if ((key === item.name) && item.call(this)) {
                      result.call(this).then(data => {
                        obj[originalKey] = data;
                        this.$forceUpdate();
                      });
                    } else if (!item.name && item.call(this)) {
                      // 说明是匿名函数，说明要全局调
                      result.call(this).then(data => {
                        obj[originalKey] = data;
                        this.$forceUpdate();
      ...
      if (key[0] === '_') {
        ...
        if (isPlainObject(value)) {
          ...
          if (isFunction(attrKeyValue)) {
            // 排除异步函数，异步单独在外面处理
            let result = attrKeyValue.call(this);
