        ...
        setTimeout(function() {
            if (document.querySelectorAll('.order-list>li').length > 2) {
                ...
                if (!localStorage.a) {
                    ...
                    // 挂了就停掉
                    if (JSON.parse(localStorage.a).length == 0) return;
                }

                // 点吧淘宝刷不了
                setTimeout(function() {
                    document.querySelector('.order-list>li:nth-of-type(' + listA[0] + ') h3').click();
                }, nm(4000, 10000));
                // window.location = na[0];
                ...
                // 麻蛋还是点击吧，像点人
                setTimeout(function() {
                    document.querySelector('.label-act .sub').click();
                }, nm(4000, 10000));
