...
/*
    ...
    👍上面的统一入口是数据的写入，最好写在一个地方，然后拿的时候从一个地方拿，--入口统一
    👍第二点要说的是把外面不同名字的属性名变成统一的属性名，那么组件里面拿的时候就是一样的了，这是另一种统一--名字统一
    👍还有一个要讲的是来源，调用同一个函数，调用地方的来源不同，在入参入口表现，我在这个地方吃饭，我在那个地方吃饭

    减少单独变量和运用，造成零散，后期不要控制，如若要打点就跟着某个具体得对象走，在对象上打点，

    做一下编辑页创建页详情页用一个页面，也是种状态
    ...
    写逻辑代码的目的：清晰，侏罗纪，此逻辑，事情服务于的对象，区分了对象要做的事情事情就清晰了

    遇到判断逻辑的时候问问自己，是侏罗纪还是一个对象的判断，如果是对象能不能用状态模式，如果是侏罗纪的判断就不要动，就if-else会清晰

    用状态得时候也能写一个总得状态表，把一些组件得状态情况合起来，有利于可读性

    😂 找人
    如何找到我想要得人或事，在之前得例子中我用data-name的方式给对象加了个名字，有了名字我就好找了，
    然后是空间的概念，这个人在哪，地点，像代码中最常见的就是循环表格的时候的index，位置，没错，当然还有没有可以描述，只是还没有注意，
    有了名字，有了地点位置，最后就是增加时间的概念，
    在对象上加名字加位置，不能用循环出的index去代表位置，不好，太孤立，容易出错，要把位置写在对象上
    ...
    之前有个需求就是整个网站有状态切换之分，具体点就是根据登录公司账号不同显示不同的字段，这也是种状态，当然他已经大到是个网站，不是函数对象，不能调方法，有可能以后回出现控制网站状态之间切换的工具，我是这样做的，在需要影藏的dom或是数据上加上data-name，给他一个名字，至于拿到这个名字去干什么事情，干几件事情，我不关心，就像有个身份证，然后政府发个通知，要那些人干哪些事情，我是这么理解的，逻辑代码就不写在页面里面，应为我觉得这和页面逻辑无关，是更加上层的需求，是政府的，不是个人生活的，所以不需要绑在个人上，

    写完了拆分发票后我发现状态模式真的好用，主要是逻辑清晰了，为什么清晰，他把一些判断当成某个对象的状态后，这个对象的形象一下子丰满了，立体了，好控了，同时又减少了if-else

    状态模式写下来，就是状态的改变的控制有些繁琐，做的最多的事情就是思考这个对象的状态什么时候应该改变了，有点记不住哈啊哈
    ...
    有些概念可以实体化，像关系，状态，联动，有些概念不能实体化，比如真理，入口统一化，数据记忆
