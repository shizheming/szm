...
// 版本：2.1
// 时间：2016.7
// 更新：1. 增加登录页过滤抓数据的动作，登录页不抓数据
//       2. 增加与native的交互，start-遮掉页面，work-接收数据，finished-完成
//       3. 增加用户账户里没有订单的情况
//       4. 抓到的链接数组去打乱他，随机延迟4-7秒抓一单，模拟人为动作，防止被屏蔽
//       5. 增加最多抓20条的判断
//       6. 修复用户强退后，再进去验证，造成的本地存储里面a属性链接数组的undefined，以致js不运行了
//       7. 用延迟防止给native发请求并发（以后要写个队列）
//       8. 还是通过url传数据，之前是用回调函数return出去的
//       9. 数据处理优化，去掉注释，去掉input，去掉空标签，去掉空格，去除换行
//       10. 增加登录页文案引导
// ------------------------------------------------------------
...
function ct(logo) {
    ...
    cd.innerHTML = '请登录您的' + logo + '账号，开始' + logo + '认证<br>任我花不会保存你的账号密码，请放心认证';
